  ...
  return async function (dispatch) {
    try {
      const response = await axios.get(
        `/filters/products?category=${categorys}`
      );
      const category = response.data;
      dispatch({
...
export const addProductToCart = (user, product, quantity) => {
  return async (dispatch) => {
    try {
      const response = await fetch("/cart/add-product", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
...
export const incrementProductQuantity = (user, product) => {
  return async (dispatch) => {
    try {
      const response = await axios.put("/cart/addProductOne", {
        user,
        product,
      });
  ...
  return async (dispatch) => {
    try {
      const response = await fetch(
        `/api/removeProductInCart/${idProduct}/${idUser}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
